...
void Plot::makeplot(exptree* tree, PlotWindow& w, bool calculus = false) {
    ...
    int cur_plot = w.customplot->graphCount() - 1;
    if (calculus) {
        w.customplot->graph(cur_plot)->setBrush(QBrush(QColor(0, 0, 255, 20)));//积分曲线图绘制
    }
    // graph(0);可以获取某个数据曲线（按添加先后排序）
    // setData();为数据曲线关联数据

    w.customplot->graph(cur_plot)->setData(x, y);
    //w.customplot->graph(cur_plot)->setName("graph");// 设置图例名称
    // 为坐标轴添加标签
